---
title: Mac 换账号后 Ghostty 配置没了？其实是配置路径的问题
date: 2026-06-04 01:20:00
tags:
  - Ghostty
  - macOS
  - Tmux
...
categories:
  - 效率工具
description: 解释 Ghostty 在 macOS 不同账号下配置无法共用的原因，并整理复制、共享目录软链接、dotfiles 仓库三种同步方案。
---

## 摘要

同一台 Mac 上，Ghostty 在一个账号里已经配置好，切换到另一个 macOS 账号后却像全新安装一样。问题不在 Ghostty，而在 macOS 的用户目录隔离：`~/.config/ghostty/config` 默认属于当前账号。本文整理这个问题的原因，以及如何用复制、共享目录软链接和 dotfiles 仓库复用同一套配置。
